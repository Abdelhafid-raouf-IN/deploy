...
# Dossier où les fichiers de test seront générés
output_dir = "generated_tests"
os.makedirs(output_dir, exist_ok=True)

# Modèle de Background
background_template = """
  Background:
    * configure ssl = true
    #-------------------------------------[ JSON FILES CALL ]-------------------------------------#
...
def generate_tests(swagger_data):
    ...
    for endpoint, methods in paths.items():
        for method, details in methods.items():
            ...
            # Préparer le contenu du fichier de test
            scenario_content = background_template + token_request_template + rest_query_template.format(
            ...
            filename = os.path.join(output_dir, f"{scenario_name_clean}.feature")
...
def get_swagger_urls_from_db():
    ...
    cur = conn.cursor()
    cur.execute("SELECT url FROM swagger_urls")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return [row[0] for row in rows]

# Récupérer les URLs Swagger à partir de la base de données et générer les tests pour chaque URL
swagger_urls = get_swagger_urls_from_db()
for swagger_url in swagger_urls:
    response = requests.get(swagger_url)
    swagger_data = response.json()
    generate_tests(swagger_data)
